...
# All 16 video compositions in packages/remotion/src/compositions/ — mirrors
...
#
# Was missing CardConverge, PromoHighlight, ShaderDissolve — confirmed live,
# compose_batch.py already maps templates to all three (exclusive-badge ->
# PromoHighlight, shader-dissolve -> ShaderDissolve, card-converge ->
# CardConverge) and builds real props for them, but this set never got
# updated when those compositions were added to the Remotion registry, so
# every post using one of those three templates hit the ValueError below
# and never dispatched a render at all.
KNOWN_COMPOSITIONS = {
    "HeroReveal",
    "PriceReveal",
    "FabricDetail",
...
async def dispatch_video_render(post_id: str, composition_id: str, props: dict) -> str | None:
    ...
    # GitHub's own dispatches endpoint occasionally 500s transiently (confirmed
    # live) — retry a couple of times with a short backoff for 5xx only. A 4xx
    # (bad ref, bad workflow file, bad token) is a real configuration problem
    # a retry won't fix, so those still raise immediately.
    DISPATCH_MAX_RETRIES = 2
    async with httpx.AsyncClient() as client:
        for attempt in range(DISPATCH_MAX_RETRIES + 1):
            ...
            if attempt == DISPATCH_MAX_RETRIES:
                break
            logger.warning(
                "GitHub dispatch for post %s got %s (attempt %d/%d) — retrying",
                post_id, resp.status_code, attempt + 1, DISPATCH_MAX_RETRIES + 1,
            )
