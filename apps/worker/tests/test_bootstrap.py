...
async def test_step_6_verify_runs_checks_on_first_run(tmp_path):
    """On a genuine first-time run (BOOTSTRAP.md still present), step_6_verify must
    actually execute its four checks — not silently skip them and report success.

    This is the exact scenario the inverted _is_step_done(6) guard broke: on a real
    first run the marker exists, so verify-and-finish must NOT take the "already
    completed" early-return path.
    """
    import bootstrap.steps as steps_module

    marker = tmp_path / "BOOTSTRAP.md"
    marker.write_text("# Bootstrap\n", encoding="utf-8")

    with patch.object(steps_module, "REPO", tmp_path), \
         patch.object(steps_module, "BOOTSTRAP_MARKER", marker), \
        ...
        await steps_module.step_6_verify()
...
async def test_step_6_verify_skips_when_already_completed(tmp_path):
    """Once BOOTSTRAP.md has already been deleted, step_6_verify must return True
    immediately without re-running any checks."""
    import bootstrap.steps as steps_module

    marker = tmp_path / "BOOTSTRAP.md"  # deliberately not created

    with patch.object(steps_module, "REPO", tmp_path), \
         patch.object(steps_module, "BOOTSTRAP_MARKER", marker), \
         patch("agents.Runner.run") as mock_run:
        result = await steps_module.step_6_verify()
