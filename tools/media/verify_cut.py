...
Usage:
  1. render:      python tools/media/render_cuts.py <project> --style <s> --mode preview
  2. extract+asr: ffmpeg -i output/preview-<s>.mp4 -vn -ac 1 -ar 16000 work/audio/preview.wav
                  (transcribe.py was removed in the SocialFTE harvest — pending a
                  faster-whisper replacement; re-wire this step once that lands)
  3. verify:      python tools/media/verify_cut.py <project> [--rendered preview]
...
from tools.media.cutlib import AudioProbe, active_keeps, load_words, plan_clip
